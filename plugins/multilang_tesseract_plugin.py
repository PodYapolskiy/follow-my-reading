...
@register_plugin
class MultilangTesseractPlugin:
    name = "multilang_tesseract"
    languages = ["eng", "rus", "ara"]
    description = "Tesseract Open Source OCR Engine For Multiple Languages"
    ...
    def process_image(filename: str) -> ImageProcessingResult:
        model_response = pytesseract.image_to_data(
            filename,
            lang="+".join(MultilangTesseractPlugin.languages),
            output_type="dict",
        )
